...
def Profile(motifs):
    ...
    for i in xrange((len(motifs[0]))):
        for j in nucleotides:
            nucleotidesDict[i].update({j:0})

    for i in xrange((len(motifs[0]))):
        for j in xrange(len(motifs)):
            nucleotidesDict[i][motifs[j][i]] += 1
        for j in nucleotides:
            nucleotidesDict[i][j] = float(nucleotidesDict[i][j]) / float(len(motifs))
...
with open('dataset_159_5.txt') as iF, open('bioinformatics_out.txt', 'w+') as oF:
